...
export default function FavoriteButton({
  ...
  async function HandleAddFavoriteButton(event) {
    const formData = new FormData(event.target);
    const recipeData = Object.fromEntries(formData);
    const response = await fetch("/api/favorite-buttons", {
      method: "POST",
      headers: {
  ...
  async function HandleToggleFavoriteButton(event) {
    const response = await fetch(`/api/favorite-buttons/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
